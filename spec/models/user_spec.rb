...
describe User do
  describe '#create' do
    ...
    it "nameがない場合は登録できないこと" do # テストしたいことの内容
      user = build(:user, name: nil) # 変数userにbuildメソッドを使用して、factory_botのダミーデータを代入(今回の場合は意図的にnameの値をからに設定)
    ...
    it "passwordが6文字以上であれば登録できること" do
        user = build(:user, password: "111111", encrypted_password: "111111") # buildメソッドを使用して6文字のパスワードを設定
        expect(user).to be_valid
    end

    it "passwordが6文字以下であれば登録できないこと" do
      user = build(:user, password: "11111", encrypted_password: "11111") # 意図的に5文字のパスワードを設定してエラーが出るかをテスト
      user.valid?
      expect(user.errors[:encrypted_password]).to include("は6文字以上で入力してください")
    end

    it "nameが2文字以上であれば登録できること" do
      user = build(:user, name: "広島")
      user.valid?
      expect(user).to be_valid
    end

    it "nameが2文字以下であれば登録できないこと" do
      user = build(:user, name: "広")
      user.valid?
